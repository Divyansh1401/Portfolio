/* ============================================================
   ...
   Recent-groups product rule:
     1. ALL active (non-settled) groups are shown — even if > 4.
     1b. A group with NO expenses and NO settlements counts as new, not settled,
         and is always shown too (2026-08-04 fix — see renderGroups).
     2. If active+new count < 4, pad with most-recent settled groups until 4.
...
(function () {
  ...
  function renderGroups(root) {
    ...
    //   1b. Brand-new groups (zero expenses + zero settlements) are always shown.
    //   2. If active+new count < 4, pad with most-recent settled groups until 4.
    //   3. Final list ordered by createdAt desc.
    ...
    var groupsWithBal = allGroups.map(function (g) {
      // A group with no expenses AND no settlements is BRAND NEW, not "settled
      // up" — getGroupBalance() returns ₹0/'settled' for both cases and can't
      // tell them apart. Treating a new group as settled made it pad-eligible
      // only, so a just-created group vanished from Recent groups whenever 4+
      // groups were already active (padCount === 0). New groups now sit in the
      // always-shown bucket.
      var isNew = Store.getExpenses(g.id).length === 0 &&
                  Store.getGroupSettlements(g.id).length === 0;
      return { group: g, bal: Store.getGroupBalance(g.id), isNew: isNew };
    });
    var active  = groupsWithBal.filter(function (x) { return x.isNew || x.bal.direction !== 'settled'; });
    var settled = groupsWithBal.filter(function (x) { return !x.isNew && x.bal.direction === 'settled'; });
    var padCount = Math.max(0, 4 - active.length);
    // Keep the {group, bal, isNew} wrappers through to render — the card needs
    // isNew for its copy, and reusing `bal` avoids a second getGroupBalance().
    ...
    if (groups.length === 0) {
      ...
      groups.forEach(function (x) {
        var html = Render.groupCard(x.group, x.bal, !x.isNew);
